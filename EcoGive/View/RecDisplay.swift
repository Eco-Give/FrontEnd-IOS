...
//  EcoGive
//
//  Created by Mahmoud Gharbi on 14/12/2023.
//
    ...
    var body: some View {
        NavigationView {
            ...
            .onAppear {
                Task {
                    do {
                        let url = URL(string: "http://172.20.10.2:3001/api/reclamations/")!
    ...
    var body: some View {
        NavigationView {
            ...
            .onAppear {
                Task {
                    do {
                        let url = URL(string: "http://172.20.10.2:3001/api/news/")!
                        let (data, _) = try await URLSession.shared.data(from: url)
...
struct AddReclamationView: View {
    ...
    private func addReclamation() async {
            do {
                let url = URL(string: "http://172.20.10.2:3001/api/reclamations/store")!
    ...
    var body: some View {
        VStack(alignment: .leading) {
            ...
            List {
                ...
                NavigationLink(destination: AddReclamationView(), tag: 2, selection: $selection) {
                    Label("Add Reclamation", systemImage: "plus.circle.fill")
                }
